    ...
    async def study_mode(self, ctx):
        ...
        async with ctx.channel.typing():
            ...
            if not check:
                ...
                await ctx.send(content=ctx.author.mention, embed=SimpleEmbed("Study mode is now on"))
            else:
                if check[0][0]:
                    await self.client.db.change_study_mode(ctx.author.id, False)
                    await ctx.send(content=ctx.author.mention, embed=SimpleEmbed("Study mode is now off"))
                else:
            ...
            if record:
                ...
                for author_id in record:
                    if message.author.id == author_id["user_id"] and message.guild:
                        ...
                        await message.channel.send(content=message.author.mention, embed=SimpleEmbed("Please go and study!"))
                    if author_id["user_id"] in mentions and message.guild:
                        await message.delete()
                        await message.channel.send(content=message.author.mention, embed = SimpleEmbed(f"{self.client.get_user(author_id[0])} is studying please dont disturb him :)"))
