...
class dictionary(commands.Cog):
    ...
    async def synonym(self, ctx, arg):
        Synonyms = str(dictionary.synonym(arg)).replace("[", "").replace("]", "").replace("'", "").split(",") # getting the synonyms and putting them in a list (while removing any characters that are not needed)
