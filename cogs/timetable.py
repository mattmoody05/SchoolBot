...
class TimeTable(commands.Cog):
    ...
    async def insert(self, ctx, day: str, time: str, *, text: str):
        ...
        if time.count(":") < 1:
            ...
            return
        try:
            await ctx.author.send(f"Time set for - {' '.join([day, valid_time, work])}")
            open_dms = True
        except discord.Forbidden:
            await ctx.send("You cant use this because I need to DM you :), please change your Privacy settings to use this Command!")
            open_dms = False
